defmodule Blockchain.Blocktree do
  ...
  """
  @spec new_tree(Block.t) :: t
  def new_tree(gen_block, hash \\ nil) do
    # TODO: Test this pre-passed hash
    hash = hash || Block.hash(gen_block)
  ...
  def add_block(blocktree, block) do
    ...
    new_tree = case get_path_to_root(blocktree, block.header.parent_hash) do
    ...
    %{new_tree | parent_map: Map.put(new_tree.parent_map, block_hash, block.header.parent_hash)}
    ...
    case path do
      ...
      [path_hash|rest] ->
        case blocktree.children[path_hash] do
          nil -> raise "Invalid path to root" # this should be impossible unless the tree is missing nodes
          sub_tree ->
  ...
  def get_path_to_root(blocktree, hash) do
    # TODO: Reverse, etc
    do_get_path_to_root(blocktree, hash)
  end
